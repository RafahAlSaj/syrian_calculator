...
const USD_TRY_DISPLAY_DECIMALS = 2;
...
function setPriceFieldsFromNew(newValue) {
    ...
    inputs.priceUsd.value = usdValue > 0 ? formatNumericStringWithGrouping(String(roundToDecimals(usdValue, DISPLAY_DECIMALS))) : "";
    inputs.priceTry.value = tryValue > 0 ? formatNumericStringWithGrouping(String(roundToDecimals(tryValue, DISPLAY_DECIMALS))) : "";
...
function syncPriceFromField(sourceField) {
    ...
    if (sourceField === "priceOld") {
        ...
        if (!(usdRate > 0)) {
            ...
            inputs.priceNew.value = "";
            inputs.priceTry.value = "";
            inputs.priceUsd.value = sourceValue > 0 ? formatNumericStringWithGrouping(String(roundToDecimals(sourceValue, DISPLAY_DECIMALS))) : "";
            return;
        }
        ...
        if (!(tryRate > 0)) {
            ...
            inputs.priceNew.value = "";
            inputs.priceUsd.value = "";
            inputs.priceTry.value = sourceValue > 0 ? formatNumericStringWithGrouping(String(roundToDecimals(sourceValue, DISPLAY_DECIMALS))) : "";
            return;
        }
...
function recalc() {
    ...
    const paidUsdRounded = roundToDecimals(paidUsd, DISPLAY_DECIMALS);
    const paidTryRounded = roundToDecimals(paidTry, DISPLAY_DECIMALS);
